...
import kr.ac.konkuk.movieratingreviewservice.databinding.ItemFeaturedMovieBinding
import kr.ac.konkuk.movieratingreviewservice.databinding.ItemMovieBinding
...
class HomeAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    //섹션 헤더 반환
    //반환한 Item View Type을 토대로 프래그먼트에서 spanCount 를 계산
    //onCreateViewHolder 에서 어떤 뷰홀더를 쓸지 결정
    ...
    //데이터를 조립하여 넣음
    //데이터를 1대1로 매핑해서 보여주는 것이 아닌 섹션 헤더나 버튼 같은 별개의 아이템을 리스트 요소 사이에 넣어주고 싶을 때 해결 방법
    //1. 헤더의 존재 수 만큼 인덱스가 미룸 - 보정 필요, 첫번째에 있는 경우엔 쉬울 수 있으나 복수의 헤더가 사이사이에 존재할 경우 보정의 난이도가 높아짐
    //2. header 도 list 에 포함을 시켜버려서 index 를 고려하지 않아도 되도록 설정 - 보정 필요x , 부가적인 가공 필요
